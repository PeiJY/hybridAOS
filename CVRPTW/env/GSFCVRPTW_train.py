...
class GSFCVRPTWenv(gym.Env):
    def __init__(self, instances=None, max_NoT=50, max_runtime=600, nPop=100,shuffle_instance = False,name = 'dqn_GSF'):
        self.log_folder = './' +name+'_train_log'
    ...
    def step(self, action):
        ...
        if stop:
            print(min(self.init_fitnesses) - min(self.fitnesses))
            f = open(self.log_folder+'/episode_log','a')
            f.write(str(self.instance) + ' ' +str(self.cumulated_r)+' '+str(min(self.fitnesses)) +' '+str(min(self.init_fitnesses)-min(self.fitnesses))+ '\n')
            f.close()
    ...
    def reset(self):
        ...
        print('--- reset ', self.reset_count,', chagne to istance ',str(self.instance),' ---')
        

        self.reset_count += 1
        self.instance_index = self.reset_count % len(self.instances)
