...
from envs.ea.stateless_aos import random_select
...
# g_folder = './temp1/'
# g_folder = './test_all_seed/'
g_folder = './temp2/'

class DEEnv(gym.Env):
    def __init__(self, fun, lbounds, ubounds, dim, best_value,benchmark_name = '',online_update = False, check_overwrite_log = True, init_random_seed=0,eps=0.1,folder=None,\
        ...
        self.max_budget = 1e4
        self.reset_count = 0
        self.ope_use_count = [0,0,0,0]
        self.w_avg = self.SLaos.w
        self.print_debug = 1  # 2: detail W an R | 1: 10 section avg W and # operator use | others: no print
        ...
        self.inner_reward_sum = 0
        if folder == None:
            folder = g_folder
        if self.write_log and not os.path.exists(folder):
            os.mkdir(folder)
        if benchmark_name == 'cec2005':
            folder += 'cec_2005_test_log/'
            if self.write_log and not os.path.exists(folder):
                os.mkdir(folder)
            folder += str(self.fun)+'_'+str(dim)
        elif benchmark_name == 'cec2017':
            folder += 'cec_2017_test_log/'
            if self.write_log and not os.path.exists(folder):
                os.mkdir(folder)
            folder += str(self.fun)+'_'+str(dim)
        elif benchmark_name == 'bbob2023':
            folder += 'bbob_2023_test_log/'
            if self.write_log and not os.path.exists(folder):
                os.mkdir(folder)
            folder += str(self.fun)+'_'+str(dim)
        else:
            raise Exception(benchmark_name, fun.__module__.split('.')[0],' -> function lib is not in BBOB2023, CEC2005 and CEC2017')
    ...
    def step(self, action):
        ...
        use_joint = self.use_joint        
        log_file = self.log_file
        # -------- joint stateless and state-based --------
        if use_joint:
            if use_random and self.SLaos.wl==1 and self.SLaos.wu==1:
                prob = [0.25,0.25,0.25,0.25]
                action = random_select(prob)
            else:
        ...
        # -------- joint stateless and state-based --------
        r = self.SLaos.calculate_reward(new_dist=self.F1[self.i],origin_dist=self.copy_F[self.i])

        # --------------------- end -----------------------
        if self.print_debug == 1:
            if self.budget% (self.max_budget/ 10) == 0:
                print(self.ope_use_count,self.w_avg/((self.max_budget/ 10)))
                self.w_avg = 0
                self.ope_use_count = [0,0,0,0]
        # if self.print_debug == 2:
            # print(self.SLaos.CA, self.SLaos.OS, str(self.SLaos.wu), str(self.SLaos.wl), str(self.SLaos.Alpha), str(self.SLaos.Beta),' | ',self.SLaos.w,' | ', self.SLaos.P,' | ', chosen_AOS,' |', action, ' | ',r)
        reward = 0
        ...
        if self.award_global_improvement:
            r = r * reward
        if use_joint and not use_random:
            
            self.SLaos.update_select_probs(r,action)
            improved = (r>0)
            if self.weight_update_rule == 2:
                self.w_avg += self.SLaos.weight_update2(chosen_AOS,improved)
            elif self.weight_update_rule == 3:
                self.w_avg += self.SLaAOSs.update_select_probs(r,chosen_AOS)[0]
            else:
                self.w_avg += self.SLaos.weight_update(improved)
        # --------------------- end -----------------------
        ...
        self.inner_reward_sum += reward
        if self.budget <= 0:
            if self.write_log:
            
                logf = open(log_file,'a')
            
                logf.write( str(self.best_so_far) + '\n')
                logf.close()
                
            #self.file.write('Episode finished \n');
            # print("\n$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$",self.budget, self.best_value, self.best_so_far,"$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$\n")
            self.c += 1
            print(' ==== ',self.best_so_far, self.inner_reward_sum, ' ====')
    ...
    def reset(self):
        ...
        # self.budget = 5e4 #* self.dim
        # self.max_budget =5e4 #* self.dim

        self.generation = 0
        self.NP = 100 #* self.dim
        self.reset_count += 1
        random.seed(self.reset_count + self.init_random_seed)
        np.random.seed(self.reset_count + self.init_random_seed)
        print('--- random seed: ', self.reset_count + self.init_random_seed,' ---')
        self.X = self.lbounds + ((self.ubounds - self.lbounds) * np.random.rand(self.NP, self.dim))
        # print(self.fun.objective_function(self.X[0]))
        # print(self.fun(self.X[0]))
        self.fun(self.X[0])
        ...
        print('  ------, reset, ', self.log_file, self.best_so_far,' ------')
...
class DEEnv_ext_slaos(gym.Env):
    def __init__(self, fun, lbounds, ubounds, dim, best_value,benchmark_name = '',online_update = True, check_overwrite_log = True, init_random_seed=0,folder=None,\
                slaos=None,use_random = True,use_joint = True,write_log = True,award_global_improvement = True,weight_update_rule = 1):
        self.n_ops = 4
        self.action_space = spaces.Discrete(self.n_ops)
        self.observation_space = spaces.Box(-np.inf, np.inf, shape=(99,), dtype = np.float32)
        self.max_gen = 10
    ...
    def step(self, action):
        ...
        use_joint = self.use_joint        
        log_file = self.log_file
        # -------- joint stateless and state-based -------- # moved to jointgreedyQpolicy
        # if use_joint:
        #     if use_random and self.SLaos.wl==1 and self.SLaos.wu==1:
        #         prob = [0.25,0.25,0.25,0.25]
        #         action = random_select(prob)
        ...
        # -------- joint stateless and state-based -------- # moved to dqn agent
        stateless_r = reward
        if self.award_global_improvement:
            stateless_r = stateless_r * reward
        if use_joint and not use_random:
            self.w_avg += self.SLaos.w
        # --------------------- end -----------------------
        ...
        if self.budget <= 0:
            ...
            # if self.c == 25:
                # print("\n$$$$$$$$$$$$$$$$$$$$$$$$$best value = ",self.best_value,"mean best found = ", self.best_found / self.c ,"$$$$$$$$$$$$$$$$$$$$$$$$$$$$\n")
            # file.close()
            return ob, reward, True, {},stateless_r
        else:
            return ob, reward, False, {},stateless_r
